...
export class EmployeeService {
  ...
  private readonly base = `${EMP_BASE_URL}${API_ENDPOINTS.employees}`;
  ...
  getEmployeeById(id: string): Observable<Employee> {
    return this.http.get<Employee>(`${EMP_BASE_URL}${API_ENDPOINTS.employeeById(id)}`);
  }

  updateEmployee(id: string, request: UpdateEmployeeRequest): Observable<Employee> {
    return this.http.put<Employee>(`${EMP_BASE_URL}${API_ENDPOINTS.employeeById(id)}`, request);
  }

  deleteEmployee(id: string): Observable<void> {
    return this.http.delete<void>(`${EMP_BASE_URL}${API_ENDPOINTS.employeeById(id)}`);
  }

  getEmployeeByCode(code: string): Observable<Employee> {
    return this.http.get<Employee>(`${EMP_BASE_URL}${API_ENDPOINTS.employeeByCode(code)}`);
  }

  getByDepartment(departmentId: string): Observable<Employee[]> {
    return this.http.get<Employee[]>(
      `${EMP_BASE_URL}${API_ENDPOINTS.employeesByDepartment(departmentId)}`,
    );
  }

  getByDesignation(designationId: string): Observable<Employee[]> {
    return this.http.get<Employee[]>(
      `${EMP_BASE_URL}${API_ENDPOINTS.employeesByDesignation(designationId)}`,
    );
  }

  getByManager(managerId: string): Observable<Employee[]> {
    return this.http.get<Employee[]>(
      `${EMP_BASE_URL}${API_ENDPOINTS.employeesByManager(managerId)}`,
    );
  }

  getByStatus(status: EmploymentStatus): Observable<Employee[]> {
    return this.http.get<Employee[]>(`${EMP_BASE_URL}${API_ENDPOINTS.employeesByStatus(status)}`);
  }

  updateStatus(id: string, request: UpdateStatusRequest): Observable<Employee> {
    return this.http.patch<Employee>(`${EMP_BASE_URL}${API_ENDPOINTS.employeeStatus(id)}`, request);
  }
  ...
  getAllEmployees(): Observable<Employee[]> {
    return this.http.get<Employee[]>(`${EMP_BASE_URL}${API_ENDPOINTS.employeesAll}`);
  }
}
